 ...
 * 
 */
public class RepeatData extends FOPData implements List<GroupData> {
   ...
   * @param idx 指定的序号
   * @return 要获取的分组数据对象
   * @throws InvalidArgumentException 序号超过重复数据对象的长度时, 抛出参数异常错误
   */
  public GroupData getGroupDataAt(int idx) {
    if (idx >= this.datas.size()) {
      throw new InvalidArgumentException("idx out of bound while get dataElement from RepeatData!");
    }
  ...
  /* (non-Javadoc)
   * @see com.chinamworld.mbs.data.MBSData#clone()
   */
  @Override
  ...
  /* (non-Javadoc)
   * @see com.chinamworld.mbs.data.MBSData#reset()
   */
  @Override
  public void reset() {
  }
  ...
  /* (non-Javadoc)
   * @see com.chinamworld.mbs.data.MBSData#toString()
   */
  @Override
  ...
  /* (non-Javadoc)
   * @see com.chinamworld.mbs.data.MBSData#toString(int)
   */
  @Override
  public String toString(int tabCount) {
    ...
    if (this.datas.size() > 0) {
      for (int i = 0; i < this.datas.size(); i++) {
        FOPData element = this.datas.get(i);
  ...
  /* (non-Javadoc)
   * @see com.chinamworld.mbs.data.MBSData#setAppend(java.lang.String)
   */
  @Override
  ...
  public boolean addAll(Collection<? extends GroupData> c) {
    ...
    while (list.hasNext()) {
      ...
      if (obj instanceof FOPData) {
        ((FOPData) obj).setAppend(isAppend());
      }
    }
